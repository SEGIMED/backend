...
const searchCommercialDrugsHandler = async (searchCommercialId)=>{
    try {
        ...
        const presentation = await searchDrugDetailPresentationHandler(searchCommercialId)
        return ({result,presentation})
    } catch (error) {
        throw new SegimedAPIError("Ocurrio un error al encontrar el médicamento", 500)
