...
export const createRequestHandler = async (physicianId, patientId) => {
  try {
    ...
    if (patient.treatingPhysician) {
      throw new Error("El paciente ya tiene un médico tratante.");
    }
    const existingRequest = await RequestTreatingPhysician.findOne({
      where: {
        patient: patientId,
        status: "Pendiente",
      },
    ...
    const newRequest = await RequestTreatingPhysician.create({
      patient: patientId,
