...
import welcomeEmailHtml from "../../utils/emailTemplates/welcomeEmailHtml.js";
...
const otpMailValidationHandler = async (body) => {
  ...
  if (user && userOneTimePassword) {
    ...
    const comparingBcryptOtp = await bcrypt.compare(
      body.temporaryCode,
      userOneTimePassword.temporaryCode
    );
    if (comparingBcryptOtp) {
      ...
      //Se envía el mail de bienvenida
      const emailSent = welcomeEmailHtml(user.name);
      await sendMail(user.email, emailSent, "¡Bienvenido a Segimed!");
      return cleanUser;
    }
  }
