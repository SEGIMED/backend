const validateDrugCreationData = (body) => {
  ...
  if (!drugName || typeof drugName !== "string") {
    errors.push(
      "El nombre de la droga es requerido y debe ser una cadena de texto válida."
    );
  }
  ...
  if (!measureUnitId || typeof measureUnitId !== "number") {
    errors.push(
      "El ID de la unidad de medida es requerido y debe ser un número."
    );
  }

  if (!routeOfAdministrationId || typeof routeOfAdministrationId !== "number") {
    errors.push(
      "El ID de la vía de administración es requerido y debe ser un número."
    );
  }
